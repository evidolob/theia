...
import { LanguageId, LanguageIdentifier, NULL_MODE_ID, NULL_LANGUAGE_IDENTIFIER } from '../common/modes';
import { ModesRegistry, ILanguageExtensionPoint } from '../common/modesRegistry';
...
export class LanguagesRegistry {
    ...
    private _mergeLanguage(resolvedLanguage: IResolvedLanguage, lang: ILanguageExtensionPoint): void {
        ...
        if (langAliases !== null) {
            for (let i = 0; i < langAliases.length; i++) {
                if (!langAliases[i] || (<string>langAliases[i]).length === 0) {
                    continue;
                }
                resolvedLanguage.aliases.push(<string>langAliases[i]);
            }
        }
    ...
    public getModeIdsFromFilenameOrFirstLine(filename: string, firstLine?: string): string[] {
        ...
        const mimeTypes = mime.guessMimeTypes(filename, firstLine);
